/**
 * Builds two demo tournaments through the real API and plays them out to
 * mid-event, for trying the app and for screenshots:
 *   - a youth event grouped by weight (round robins) on 3 mats
 *   - a high school invitational with seeds and double elimination on 4 mats
...
async function youthEvent() {
  ...
  for (const team of teams) {
    ...
    await call("POST", `/events/${slug}/entries/import`, undefined, { team, contactEmail: `coach@${team.split(" ")[0]!.toLowerCase()}.org`, rows });
  }
  ...
  return { slug, directorToken, info };
}
...
const youth = await youthEvent();
const hs = await highSchoolEvent();
await app.notifications.settle();
const base = process.env.PUBLIC_BASE_URL ?? "http://localhost:5173";
console.log(`Demo Kids Classic:            ${base}/e/${youth.slug}`);
console.log(`  director: ${directorUrl(youth.slug, youth.directorToken)}`);
console.log(`Demo High School Invitational: ${base}/e/${hs.slug}`);
console.log(`  director: ${directorUrl(hs.slug, hs.directorToken)}`);
console.log(JSON.stringify({ youth: { slug: youth.slug, token: youth.directorToken }, hs: { slug: hs.slug, token: hs.directorToken } }));
await app.close();
await sql.end();
